  ...
  try {
    ...
    feedback.forEach((item, index) => {
      if (!mongoose.Types.ObjectId.isValid(item.studentId)) {
        console.warn(`Record ${index} (ID: ${item._id}) has invalid studentId: ${item.studentId} (Type: ${typeof item.studentId})`);
      }
      if (!mongoose.Types.ObjectId.isValid(item.orderId)) {
        console.warn(`Record ${index} (ID: ${item._id}) has invalid orderId: ${item.orderId} (Type: ${typeof item.orderId})`);
      }
      if (!mongoose.Types.ObjectId.isValid(item.vendorId)) {
        console.warn(`Record ${index} (ID: ${item._id}) has invalid vendorId: ${item.vendorId} (Type: ${typeof item.vendorId})`);
      }
    });
