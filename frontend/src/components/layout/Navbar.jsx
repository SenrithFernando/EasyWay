          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <Link
                key={link.name}
                to={link.path}
                className={`text-sm font-medium transition-colors hover:text-brand-500 ${isActive(link.path) ? "text-brand-500" : "text-surface-600"}`}
              >
                {link.name}
              </Link>
            ))}
            ...
            <div className="px-4 py-6 flex flex-col gap-4">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`block px-4 py-3 rounded-xl text-base font-medium ${isActive(link.path) ? "bg-brand-50 text-brand-600" : "text-surface-700 hover:bg-surface-50"}`}
                >
                  {link.name}
                </Link>
              ))}
